...
static void option_DumpSyslogs(void)
{
    ...
    while (FSA_ReadDir(fsaHandle, dir_handle, &dir_entry) >= 0)
    {
        ...
        snprintf(src_path, sizeof(src_path), "/vol/system/logs/"
                                             "%s",
                 dir_entry.name);
        snprintf(dst_path, sizeof(dst_path), "/vol/storage_recovsd/logs/"
                                             "%s",
                 dir_entry.name);
...
static void option_StartWupserver(void)
{
    ...
    for (int i = 0; i < wait_time; i++)
    {
        ...
        usleep(1000 * 1000);
        gfx_draw_rect_filled(0, index, SCREEN_WIDTH, 8, COLOR_BACKGROUND);
        gfx_printf(16, index, 0, "Waiting for network connection... %ds", (wait_time - (i + 1)));
    }
...
static void option_InstallWUP(void)
{
    ...
    int dir_handle;
    int res = FSA_OpenDir(fsaHandle, "/vol/storage_recovsd/install", &dir_handle);
    if (res < 0)
    {
        gfx_set_font_color(COLOR_ERROR);
        gfx_printf(16, index, 0, "Failed to open install dir: %x", res);
        waitButtonInput();
        return;
    }
    ...
    while (FSA_ReadDir(fsaHandle, dir_handle, &dir_entry) >= 0)
    {
        if (dir_entry.stat.flags & DIR_ENTRY_IS_DIRECTORY)
        {
            snprintf(wup_path, sizeof(wup_path), "/vol/storage_recovsd/install/%s", dir_entry.name);
            MCPInstallInfo info;
            res = MCP_InstallGetInfo(mcpHandle, wup_path, &info);
            if (res < 0)
            {
                gfx_set_font_color(COLOR_ERROR);
            ...
            res = MCP_InstallTitle(mcpHandle, wup_path);
            if (res < 0)
            {
                gfx_set_font_color(COLOR_ERROR);
